...
function Awards() {
  ...
  return (
    <section ref={containerRef} className="relative py-24 md:py-40 bg-transparent overflow-hidden">
            ...
            {awardsData.map((award, i) => (
              <div key={i} className="group relative flex-shrink-0">
                <div className="flex items-end gap-6 cursor-none">
                  <span className="text-[10vw] font-display text-white/5 transition-colors group-hover:text-white group-hover:skew-x-[-10deg] duration-700 font-light uppercase leading-[1.1] tracking-[0.1em]">
                    {award.name.split(' ')[0]}
                  </span>
                  <div className="mb-4">
                ...
                <div className="absolute top-1/2 left-full ml-12 -translate-y-1/2 w-80 p-8 glass-card rounded-[32px] opacity-0 group-hover:opacity-100 transition-all duration-700 pointer-events-none z-20">
                   ...
                   <h3 className="text-white text-xl font-display mb-4 font-light uppercase leading-[1.1] tracking-[0.1em]">{award.name}</h3>
        ...
        {awardsData.map((award, i) => (
          <div key={i} className="p-8 rounded-[32px] glass border border-white/5 flex items-start justify-between group">
            <div>
              ...
              <h3 className="text-white text-xl font-display mb-1 font-light uppercase leading-[1.1] tracking-[0.1em]">{award.name}</h3>
